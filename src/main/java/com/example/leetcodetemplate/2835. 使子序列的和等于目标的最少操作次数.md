2835. 使子序列的和等于目标的最少操作次数。 第 360 场周赛 第3题。
...
//用数组 替换掉 TreeMap、Map，因为元素只有2的幂，只有 2^0 到 2^30 共31个元素。但代码量减少的不多，感觉即使有这思路 也难在周赛里敲出来。
//这题 加深了 对int的32位的理解。int范围是 [-2^31, 2^31-1]，0位到30位的每一个1都是一个2次幂。
class Solution { 
